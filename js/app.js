...
class App {
  ...
  // -----------------------------------------------------------------------------
  async generateThumbnails(models) {
    ...
    const thumbPromises = models.map(async (file) => {
      const thumbURL = await makeThumbnail(file.url);
  ...
  async showBrowseInterface() {
    ...
    try {
      ...
      // Generate thumbnails
      const thumbs = await this.generateThumbnails(files);

      // Now hide loading after thumbnails are ready
      if (loadingOverlay) loadingOverlay.style.display = 'none';

      this.createBrowseModal('Browse Models', thumbs);
    } catch (error) {
      if (loadingOverlay) loadingOverlay.style.display = 'none';
  ...
  // -----------------------------------------------------------------------------
  // Grouped Browser for Projects
  // -----------------------------------------------------------------------------
  async showGroupedBrowseInterface(project, groupedModels) {
    const loadingOverlay = document.getElementById('loading-overlay');
    if (loadingOverlay) loadingOverlay.style.display = 'flex';
    ...
    try {
      ...
      const groupedThumbs = {};
      for (const productKey in groupedModels) {
        const models = groupedModels[productKey];
        groupedThumbs[productKey] = await this.generateThumbnails(models);
      }

      // Hide loading
      if (loadingOverlay) loadingOverlay.style.display = 'none';

      this.createGroupedBrowseModal(project, groupedThumbs);
    } catch (error) {
      if (loadingOverlay) loadingOverlay.style.display = 'none';
      console.error("Error loading project models:", error);
      alert("Error accessing project models. Please check the console for details.");
    }
  }
  // -----------------------------------------------------------------------------
  // Create Flat Browse Modal
  // -----------------------------------------------------------------------------
  createBrowseModal(titleText, thumbs) {
    ...
    const description = document.createElement('p');
    description.textContent = thumbs.length === 0 ? 'No models found.' : 'Select models (multiple selections allowed): ';
    ...
    loadButton.addEventListener('click', async () => {
      const selected = [];
      modalContainer.querySelectorAll('input[type="checkbox"]:checked').forEach(cb => {
  ...
  // -----------------------------------------------------------------------------
  // Create Grouped Browse Modal for Projects
  // -----------------------------------------------------------------------------
  createGroupedBrowseModal(project, groupedThumbs) {
    const modalOverlay = document.createElement('div');
    modalOverlay.style.position = 'fixed';
    modalOverlay.style.top = '0';
    modalOverlay.style.left = '0';
    ...
    let hasModels = false;
    for (const productKey in groupedThumbs) {
      const thumbs = groupedThumbs[productKey];
      if (thumbs.length > 0) {
        hasModels = true;
        const productHeader = document.createElement('h3');
        productHeader.textContent = productKey.replace(/_/g, ' ');
        productHeader.style.marginTop = '20px';
        productHeader.style.marginBottom = '10px';
        groupsContainer.appendChild(productHeader);
    ...
    modalContainer.appendChild(groupsContainer);
  ...
  onSelectEvent(event) {
    if (this.isPlacingProduct && this.hitTestSource && event.frame) {
